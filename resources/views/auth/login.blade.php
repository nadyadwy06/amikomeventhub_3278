...
<body class="bg-indigo-900 text-white min-h-screen flex items-center justify-center p-6">
    ...
    <div class="max-w-md w-full bg-white text-slate-900 rounded-[2rem] p-8 shadow-2xl">
        ...
        @if (session('error'))
            <div class="bg-red-100 text-red-700 p-4 rounded-xl mb-4 font-medium text-sm">
                {{ session('error') }}
            </div>
        @endif

        @if (session('success'))
            <div class="bg-green-100 text-green-700 p-4 rounded-xl mb-4 font-medium text-sm">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="relative my-6">
            ...
            <div class="relative flex justify-center text-xs uppercase">
                <span class="bg-white px-3 text-slate-400 font-bold tracking-wider">Atau</span>
            </div>
        </div>

        <!-- Tombol SSO Google (Fitur 1 - Socialite) -->
        <a href="{{ route('auth.google') }}" class="w-full flex items-center justify-center gap-3 bg-white border-2 border-slate-200 text-slate-700 font-bold py-4 px-5 rounded-2xl shadow-sm hover:bg-slate-50 hover:border-slate-300 transition text-base">
